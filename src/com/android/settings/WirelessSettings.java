...
public class WirelessSettings extends RestrictedSettingsFragment
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (!"yes".equals(g3_display.toLowerCase()) && !"true".equals(g3_display.toLowerCase())) {
            getPreferenceScreen().removePreference(findPreference(KEY_WIRELESS_DEVICE_SUPPORT_INFO));
        }
        String mAirplaneModeEnabler_display = SystemProperties.get("ro.airplanemode.display","yes");
        if (!"yes".equals(mAirplaneModeEnabler_display.toLowerCase()) 
                && !"true".equals(mAirplaneModeEnabler_display.toLowerCase())) {
        ...
        if (isSecondaryUser || Utils.isWifiOnly(getActivity())
                || SystemProperties.get("ro.settings.mobilenetworks", "true").equals("false")) {
        ...
        if (isSecondaryUser || !cm.isTetheringSupported()
                || SystemProperties.get("ro.settings.hotspot", "true").equals("false")) {
