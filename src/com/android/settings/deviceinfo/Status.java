...
public class Status extends PreferenceActivity {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if (mPhone == null || Utils.isWifiOnly(getApplicationContext()) 
                || SystemProperties.get("ro.settings.phonestatus", "true").equals("false")) {
    ...
    private void setWifiStatus() {
        ...
        File wifimactxt = new File("/data/wifimac.txt");
        if(wifimactxt.exists()){
            Log.d("[leo_zt]","wifimac.txt is exist");
            try{
                InputStream instream = new FileInputStream(wifimactxt);
                if( instream != null){
                       BufferedReader br = new BufferedReader(new InputStreamReader(instream));
                       macAddress = br.readLine();
                       macAddress = macAddress.substring(0,17);
                       Log.d("[leo_zt]","macAddress : " + macAddress);
                       instream.close();
                }
	    }
            catch (IOException e){
	        System.out.println("Read wifimac.txt fail");
            }
        } else {
            Log.d("[leo_zt]","wifimac.txt isn't exist");
        }
