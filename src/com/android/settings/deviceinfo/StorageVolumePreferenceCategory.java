...
public class StorageVolumePreferenceCategory extends PreferenceCategory {
    ...
    private StorageVolumePreferenceCategory(Context context, StorageVolume volume) {
        ...
        mIsRemovable = mVolume != null ? mVolume.isRemovable() : false;
        mUhost = false;
        if(mIsRemovable){
            String mountpoint = mVolume.getPath();
            if(mountpoint.startsWith(Environment.getUhostStorageDirectory().getPath())){
                mUhost = true;
            }
        }   
    }
    ...
    public void init() {
        ...
        if (mIsRemovable) {
            ...
            if(mUhost){
                mMountTogglePreference.setTitle(R.string.uh_eject);
                mMountTogglePreference.setSummary(R.string.uh_eject_summary);
            }else{
    ...
    private void resetPreferences() {
        ...
        addPreference(mItemTotal);
        addPreference(mItemAvailable);
        if(mIsRemovable && mMountTogglePreference != null) {
            addPreference(mMountTogglePreference);
    ...
    private void updatePreferencesFromState() {
        ...
        if(mIsRemovable){
            if(mUhost){
                mFormatPreference.setTitle(R.string.uh_format);
                mFormatPreference.setSummary(mResources.getString(R.string.uh_format_summary));
            }else{
                mFormatPreference.setTitle(R.string.sd_format);
            ...
            if (Environment.MEDIA_MOUNTED.equals(state)
                ...
                if(mUhost){
                    mMountTogglePreference.setTitle(mResources.getString(R.string.uh_eject));
                    mMountTogglePreference.setSummary(mResources.getString(R.string.uh_eject_summary));
                }else{
                ...
                if(mUhost){
                    mMountTogglePreference.setTitle(mResources.getString(R.string.uh_mount));
                    mMountTogglePreference.setSummary(mResources.getString(R.string.uh_mount_summary));
                }else{
            ...
            if (mUsbConnected && (UsbManager.USB_FUNCTION_MTP.equals(mUsbFunction) ||
                        UsbManager.USB_FUNCTION_PTP.equals(mUsbFunction)) && !mUhost) {
